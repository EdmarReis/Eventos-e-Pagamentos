...
class AddCompromissoModel extends StatefulWidget {
  const AddCompromissoModel({super.key});

  @override
  State<AddCompromissoModel> createState() => _AddCompromissoModelState();
}

class _AddCompromissoModelState extends State<AddCompromissoModel> {
  String _compromissoType = "REPETICAO";
  final TextEditingController _descricaoController = TextEditingController();
  final TextEditingController _dataController = TextEditingController();
  final TextEditingController _intervaloRepeticaoController = TextEditingController();
  final TextEditingController _quantidadeEventosController = TextEditingController();
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      padding: EdgeInsets.only(
        ...
        right: 32,
        top: 32,
        bottom: MediaQuery.of(context).viewInsets.bottom, // quando abre o teclado joga o campo para cima para nao ficar por cima do campo
      ),
      child: SingleChildScrollView(  // era um colum, mas dava problema quando abria o teclado pois cobria os campos de textos
        child: Column(
          ...
          children: [
            ...
            TextFormField(
              ...
              inputFormatters: [FilteringTextInputFormatter.digitsOnly],
              enabled:
                  _compromissoType == "REPETICAO" || _compromissoType == "MENSAL", // Desativa quando necessário
            ),
            TextFormField(
              controller: _horarioController,
              decoration: const InputDecoration(
                labelText: "Horário",
                suffixIcon: Icon(Icons.access_time), // Ícone de relógio
              ),
              readOnly: true, // Impede digitação manual
              onTap: () async {
                ...
                if (pickedTime != null) {
                  ...
                  _horarioController.text =
                      formattedTime; // Define o valor no campo
                }
              },
            ),

            // precisa mandar tipo evento (LEMBRETE), recorrencia (REPETICAO) e usuario (Edmar)
            const SizedBox(height: 32),
            Row(
              children: [
                Expanded(
  ...
  onButtonAddClicked() async {
    if (!isLoading) {
      ...
      CompromissoEnvio compromissoEnvio = CompromissoEnvio(
        descricao: descricao, 
        data: data, 
        tipoEvento: "LEMBRETE", 
      ...
      await CompromissoService().addCompromisso(compromissoEnvio);
      closeModal();
    }
  }
